...
def create_qfn(input_size=37,output_size=4,intermediate_layers=[30,20,20,20,10],tn=1):
...
def main(env,state_size,action_size,num_time_steps_per_state):
    qfn_local = create_qfn(input_size=state_size,output_size=action_size,tn=num_time_steps_per_state).to(device)
    qfn_target = create_qfn(input_size=state_size,output_size=action_size,tn=num_time_steps_per_state).to(device)
    ...
    for i in range(1000):
        save = (save+1)%100
        scores.append(episode(env,epsilon_greedy_policy(qfn_local,eps),memory,weights))
        ...
        for train_step in range(train_steps_per_episode):
            ...
            dq = update_params(qfn_local,qfn_target,optimizer,samples,GAMMA)
...
def samples_to_tensors(samples):
    states = torch.from_numpy(np.vstack([np.hstack(e.state) for e in samples])).float().to(device)
    actions = torch.from_numpy(np.vstack([e.action for e in samples])).long().to(device)
    rewards = torch.from_numpy(np.vstack([e.reward for e in samples])).float().to(device)
    next_states = torch.from_numpy(np.vstack([np.hstack(e.state) for e in samples])).float().to(device)
...
def update_params(qfn_local,qfn_target,optimizer,samples,gamma):
    ...
    q_next_targets = qfn_target(next_states).detach().max(1)[0].unsqueeze(1) # modify to try double-dqn
    ...
    soft_update_target(qfn_target,qfn_local) # note target network is being updated
    return dq
    
def soft_update_target(qfn_target,qfn_local):
    for target_param, local_param in zip(qfn_target.parameters(), qfn_local.parameters()):
...
def epsilon_greedy_policy(qfn,eps):
    ...
    def policy(state):
        qfn.eval()
        with torch.no_grad():
            possible_actions = qfn(torch.from_numpy(np.hstack(state)).float().unsqueeze(0))
        qfn.train()
        ...
        probs = [p+greed if i==greedy_action else p for i in range(num_actions)]
...
def episode(env,policy,memory,weights):
    ...
    while True:
        ...
        if done:                                       # exit loop if episode finished
            ...
            rewards_out = {i:r for i,r in zip(range(num_actions),rewards) if r!= 0}
            print(rewards_out)
            break
    return score
